...
import type { PersonalInformationFormSchema } from '@/validations/employee/add-form.validation';

export const getPersonalInfoFields = (form: UseFormReturn<PersonalInformationFormSchema>) => {
  const dob = form.watch('dob');
  const dobValue = dob
    ? dateIntl.formatDate(dob, {
        dateFormat: 'yyyy-MM-dd',
      })
    : '';
  const fields: FormFieldItem<PersonalInformationFormSchema>[] = [
    {
      type: 'input',
      name: 'firstName',
      label: 'First Name',
...
export const getEmergencyContactFields = (form: UseFormReturn<PersonalInformationFormSchema>) => {
  const fields: FormFieldItem<PersonalInformationFormSchema>[] = [
    {
      type: 'input',
