...
export const AddEmployeeClipboardButton: FC = () => {
  const { copy, error: clipboardError } = useClipboard();

  const generateOnboardingLinkMutation = api.onboardEmployee.generateOnboardingLink.useMutation({
    onSuccess: data => {
      copy(`${window.location.origin}/onboard/employee?token=${data.token}`);
