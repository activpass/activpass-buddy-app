...
export const StepperFormActions: FC<StepperFormActionsProps> = ({
  ...
  const prevStep = useEmployeeFormStore(state => state.prevStep);

  const isLastStep = currentStep === ADD_EMPLOYEE_STEP_ID.PAYMENT_DETAILS;
  const isDisabledStep = currentStep === ADD_EMPLOYEE_STEP_ID.PERSONAL_INFORMATION;

  return (
    <div className={cn('col-span-2 flex w-full justify-between gap-2 pt-4', className)}>
      <Button
