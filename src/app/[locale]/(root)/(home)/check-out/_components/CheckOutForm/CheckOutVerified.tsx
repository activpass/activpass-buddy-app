...
import { CheckOutVerifiedSuccess } from './CheckOutVerifiedSuccess';
...
type CheckOutVerifiedProps = {
  type: TimeLogTypeEnum;
  clientResult: CheckOutResult;
  onResetResult: () => void;
};
export const CheckOutVerified: FC<CheckOutVerifiedProps> = ({
  clientResult,
  type,
  onResetResult,
}) => {
  ...
  const checkOutVerifyMutation = api.timeLogs.checkOutVerify.useMutation({
    onSuccess: () => {
  ...
  const onCheckOut = (data: OtpPinFormSchema) => {
    ...
    checkOutVerifyMutation.mutate({ orgId, phoneNumber, pin: +data.pin, type });
  };

  if (successfullyVerified) {
    return <CheckOutVerifiedSuccess onResetResult={onResetResult} type={type} />;
  }
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="mt-6">
        <Form<OtpPinFormSchema>
          ...
          fields={fields}
          onSubmit={onCheckOut}
          isSubmitting={checkOutVerifyMutation.isPending}
          hideResetButton
